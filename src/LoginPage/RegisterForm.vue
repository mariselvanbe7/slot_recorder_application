  ...
  <q-page class="flex flex-center">
    <div class="text-center">
      <q-card class="bg-transparent no-shadow">
        <q-form @submit="createLoginUser" class="q-px-sm q-mt-xl q-gutter-y-lg"
          ...
          <div class="q-pa-lg q-col-gutter-lg">
            <q-input outlined type="email" v-model="email" label="Email" class="q-my-md">
              <template v-slot:prepend>
                <q-icon name="email" color="red"></q-icon>
              </template>
            </q-input>
            <q-input outlined v-model="name" label="Username" :rules="[
              (val) =>
            ...
            <q-input outlined v-model="companyId" :type="isPwd ? 'password' : 'text'" label="companyId" :rules="[
              (val) =>
              ...
              <template v-slot:prepend>
                <q-icon name="private_connectivity" class="cursor-pointer" color="red">
                </q-icon>
              </template>
            ...
            <q-input outlined v-model="password" label="Password" :type="isPwd ? 'password' : 'text'" :rules="[
              (val) =>
          ...
          <div class="row justify-between q-px-lg q-pt-md">
            <q-btn @click="cancel" push outline size="md" no-caps style="border-radius: 10px"
              class="text-black col-4 text-weight-bold" label="Cancel"></q-btn>
            <q-btn push unelevated size="md" no-caps style="background-color: red; border-radius: 10px"
              class="text-white col-4" label="Register" type="submit"></q-btn>
          </div>
        </q-form>
      </q-card>
    </div>
